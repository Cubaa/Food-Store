...
export const ProductsPage = () => {
  ...
  productsFromApi.filter((product) => {
    ...
    })} else products.push({ ...product, category_name: "brak" });
  });
    ...
    <>
      <Container className="h-100">
        ...
        <Row className="pt-5 w-100 m-auto">
          <Col>
            {status === "succeeded" && (
              <TablePage data={products} name="products" />
            )}
            {status === "failed" && (
              <p style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", fontWeight: "bold", fontSize: "20px"}}
